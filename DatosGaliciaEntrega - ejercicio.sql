...
#------------------------------------------------------------------------
DELIMITER //
	DROP PROCEDURE IF EXISTS CorregirSuperficie //
	CREATE PROCEDURE CorregirSuperficie( IN concelloCorregir varchar( 30 ), IN nuevaSuperficie FLOAT )
		BEGIN
			UPDATE datos
			SET superficie = nuevaSuperficie 
            where concello = concellocorregir;
        END
// DELIMITER ;

#llamada a procedimiento y comprobaciones
#SELECT * FROM DATOS WHERE concello='Rizanxo';
#CALL CorregirSuperficie('Rizanxo', 60.0);
#SELECT * FROM DATOS WHERE concello='Rizanxo';
#------------------------------------------------------------------------
#	PROCEDIMIENTO PARA BORRAR LOS CONCELLOS CUYA SUPERFICIE OSCILE ENTRE UNOS VALORE MÍNIMO Y MÁXIMO PASADOS COMO PARÁMETROS
#------------------------------------------------------------------------
DELIMITER //
	DROP PROCEDURE IF EXISTS BorrarConcelloSuperficie //
	CREATE PROCEDURE BorrarConcelloSuperficie( IN superficieMin FLOAT , IN superficieMax FLOAT )
		BEGIN			
			delete from Datos
			where Superficie between superficieMin and superficieMax;
        END
// DELIMITER ;

#llamada a procedimiento y comprobaciones
#SELECT * FROM DATOS WHERE superficie='60';
#CALL BorrarConcelloSuperficie(59.9, 60.1);
#SELECT * FROM DATOS WHERE superficie='60';
#SELECT * FROM DATOS WHERE concello='Rizanxo';
#------------------------------------------------------------------------
...
#------------------------------------------------------------------------
DELIMITER //
	DROP FUNCTION IF EXISTS DiferenciaMujeresHombres //
	CREATE FUNCTION DiferenciaMujeresHombres( ComarcaBuscar varchar( 30 ))
			RETURNS float
            DETERMINISTIC
		BEGIN					
			DECLARE Diferencia float;
			SET Diferencia = (SELECT SUM(ABS( mujeres - hombres )) FROM Datos WHERE Comarca = ComarcaBuscar);
			RETURN Diferencia;			
        END
// DELIMITER ;

#llamada a función y comprobaciones (volver a introducir los datos de nuevo)
#SELECT mujeres,hombres FROM Datos WHERE Comarca='Barbanza';
#SELECT DiferenciaMujeresHombres('Barbanza');
...
DELIMITER //
	...
	CREATE FUNCTION SuperficieMediaComarca( ComarcaBuscar varchar( 30 ))
			RETURNS float
            DETERMINISTIC
		BEGIN					
			DECLARE SuperficieMedia float;
			SET SuperficieMedia = (SELECT AVG(superficie) FROM Datos WHERE Comarca=ComarcaBuscar) ;
			RETURN SuperficieMedia;			
        END
// DELIMITER ;

#llamada a función y comprobaciones
#SELECT superficie FROM Datos WHERE Comarca='Barbanza';
#SELECT SuperficieMediaComarca('Barbanza');
#------------------------------------------------------------------------
#	TRIGGER PARA QUE CADA VEZ QUE MODIFIQUEMOS UN DATO GUARDE EL VALOR ANTERIOR EN LA TABLA Datos_Modificados
#------------------------------------------------------------------------
DELIMITER //
		DROP TRIGGER IF EXISTS GuardarDatosOld //
		CREATE TRIGGER GuardarDatosOld
...
#------------------------------------------------------------------------
DELIMITER //
		DROP TRIGGER IF EXISTS GuardarDatosBorrados //
		CREATE TRIGGER GuardarDatosBorrados
        BEFORE DELETE ON Datos 
        FOR EACH ROW
        BEGIN
			INSERT INTO Datos_Borrados	
				VALUES (old.idDatos, old.Provincia, old.Comarca, old.Concello, old.Superficie, old.Mujeres, old.Hombres);			
        END
    // DELIMITER ;
    
#llamada a procedimientos que activen trigger  y comprobacines
#SELECT * FROM Datos WHERE Concello='Rizanxo';
#CALL CorregirSuperficie('Rizanxo', 60.0); 
#SELECT * FROM Datos WHERE Concello='Rizanxo';
#CALL BorrarConcelloSuperficie(59.9, 60.1);
#SELECT * FROM DATOS WHERE Concello='Rianxo';
#SELECT * FROM Datos_Borrados;

#SELECT * FROM Datos WHERE Concello='Redondela';
#CALL CorregirSuperficie('Redondela', 100.0);
#SELECT * FROM Datos WHERE Concello='Redondela';
#CALL BorrarConcelloSuperficie(99.9, 100.1);
#SELECT * FROM DATOS WHERE Concello='100';
#SELECT * FROM Datos_Borrados;
...
#------------------------------------------------------------------------
DELIMITER //
	DROP PROCEDURE IF EXISTS RestaurarDatosBorrados //
	CREATE PROCEDURE RestaurarDatosBorrados()
		BEGIN
			INSERT INTO Datos	
				SELECT * FROM Datos_Borrados;			
        END
